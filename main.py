...
TRADECENTER = pygame.transform.scale(pygame.image.load(os.path.join('assets', 'tradecenter.jpg')), (WIDTH, 200))
...
def draw_window(plane, wtc, bullets_left, bullets_right):
    ...
    WIN.blit(TURRET, (139, HEIGHT - TRADECENTER.get_height() - TURRET.get_height() + 10))
    WIN.blit(TURRET, (332, HEIGHT - TRADECENTER.get_height() - TURRET.get_height() + 10))
...
def print_message(win):
    if win:
        message = "GANASTE!!!"
        WIN.blit(WIN_IMAGE, ( WIDTH/2 - WIN_IMAGE.get_width()/2 , HEIGHT/2 - WIN_IMAGE.get_height()//2 ) )
    else:
        message = "PERDISTE!!!"
        WIN.blit(LOSE_IMAGE, ( WIDTH/2 - LOSE_IMAGE.get_width()//2 , HEIGHT/2 - LOSE_IMAGE.get_height()//2 ))
    display_message = MAIN_FONT.render(message,True,RED)
    WIN.blit(display_message,(WIDTH/2 - display_message.get_width()/2,HEIGHT/2 - display_message.get_height()/2))
    pygame.display.update()
...
def main():
    ...
    plane = pygame.Rect(0 + WIDTH/2 - PLANE.get_width()/2 ,0 ,PLANE.get_width(),PLANE.get_height())
    wtc = pygame.Rect( 0, HEIGHT - TRADECENTER.get_height(), TRADECENTER.get_width(), TRADECENTER.get_height())
    bullets_left = list()
    bullets_right = list()
    lr = False
    while run:
        ...
        for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_LSHIFT and len(bullets_left) < MAX_BULLETS:
                        bullet = pygame.Rect(139, HEIGHT - TRADECENTER.get_height() - TURRET.get_height() + 10, BULLET.get_width(), BULLET.get_height())
                        bullets_left.append(bullet)
                    if event.key == pygame.K_RSHIFT and len(bullets_right) < MAX_BULLETS:
                        bullet = pygame.Rect(332, HEIGHT - TRADECENTER.get_height() - TURRET.get_height() + 10, BULLET.get_width(), BULLET.get_height())
                ...
                if event.type == BULLET_HIT_PLANE:
                    WIN.blit(EXPLOSION, (plane.x + (plane.width /4) ,plane.y + plane.height/4 ))
                    print_message(True)
                    pygame.time.delay(2000)
                    main()
